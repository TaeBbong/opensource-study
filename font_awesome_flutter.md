...
- 접근성 정보를 제공하기 위한 위젯!
- 여기서 접근성은 시각장애인 등 음성으로 화면을 읽어줘야 하는 경우를 말하는 접근성임.
- 이것도 몰랐는데, 접근성 트리라는게 있는듯
- Semantics로 감싸면 label을 설정하여 어떤 접근성 정보를 제공해야 하는지 설정할 수 있고,
- 너무 하위의 자식, 여기서는 iconWidget과 같이 굳이 사용자에게 알려줄 필요가 없는 정보는
- 접근성 제거를 뜻하는 ExcludeSemantics으로 감싸서 접근성 트리로부터 제거
