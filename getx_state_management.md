...
> 참고) How setState() works??
>
> 리액트 강의를 보다가, setState()는 실행되는 즉시 상태 값을 변화시키는 함수가 아니라는 것이 나옴.
> 화면이 리렌더링 될 때 상태 값을 바꾸도록 예약/스케쥴링/배치 해놓는 기능이 정확한 이해라고 함.
> 
> 오 그러면 플러터의 setState()도 마찬가지인가 싶어 확인해봄.
> setState()를 호출하면 State 객체가 dirty 상태로 표시됨.
> 이런 dirty 상태의 위젯이 있으면, 플러터의 빌드 스케쥴러는 해당 위젯에 대해 다음 프레임에서 리렌더링을 예약함.
> 따라서 그 다음 프레임이 되면 해당 위젯은 리렌더링이 되며, 이 때 상태 변화가 일어남.
>
> 여기서 프레임이란? 우리가 알고 있는 60fps, 120fps에서의 프레임. 화면을 새로 고침하는 단위라고 한다.
> 일반적으로 우리가 사용하는 스마트폰은 60fps, 즉 1초에 60번 프레임이 새로 그려질 수 있다는 것.
> 여기서 중요한 것은 새로 그려진다는게 아니라 그려질 수 있다는 것.
> 그렇기에 플러터는 1초에 60번씩 스케쥴러가 예약해놓은 리렌더링이 있나 확인하고, 해당 프레임에 예약해놓은 리렌더링이 있다면 화면을 다시 그리는 것이고, 없다면 해당 프레임은 스킵, 기존 화면을 유지.
>
> 다시 요약하자면, 플러터에서 setState()를 호출하면 해당 State 객체가 그 순간 dirty 상태가 되고, 다음 프레임에서 리렌더링이 되도록 예약이 됨. 빌드 스케쥴러는 예약된 이벤트를 보고 다음 프레임에서 리렌더링을 수행하도록 함. dirty 상태가 된 위젯이 없다면, 즉 빌드 스케쥴러에 예약된게 없으면 해당 프레임은 스킵됨. 이와 같이 예약을 확인하는 행위는 1초에 60번씩 수행됨.
>
> 리액트의 setState와 일맥상통하는 내용이라 신기하기도 했다. 그럼과 동시에 GetX의 Obx가 동작하는 원리와도 유사했다. 여기서 빌드 스케쥴러가 Notifier가 된다는 것.